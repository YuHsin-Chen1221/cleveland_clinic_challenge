...
network~[10], and topology-driven signal propagation is well
established~[12]. A graph is also the natural setting for a quantum walk.

% =====================================================================
\section{Prior Work and the Gap}

\lead{Classical graph methods.} Elastic network models~[10,11] predict
fluctuations and collective modes from topology; Estrada communicability
$e^{A}$~[8] measures diffusive connectivity. The strongest published result in our
exact setting is bond-to-bond propensity analysis, which recovers allosteric sites knowing only the
orthosteric site~[3]. These are fast, MD-free and the honest baselines; their common
...
network reaching ROC-AUC $0.87$ on 39 confirmed cryptic pockets~[13]. Its constraint is
supervision: labelled apo--holo data is scarce (ASBench, a few hundred
entries~[14]; CASBench, 91 enzymes), limiting generalisation to
novel targets. A training-free method sidesteps this.

\lead{Quantum approaches.} Continuous-time quantum walks are mature~[1]; open-system
generalisations interpolate between quantum and classical walks~[5]; noise-assisted
transport is characterised in photosynthesis~[6,7]; complex edge phases
yield directed walks~[9]. Most directly, unitary CTQW centrality was recently applied to
residue networks with an IBM demonstration~[2].
...
anisotropic-network-model mode along edge $(i,j)$~[11] supplies a complex phase; ANM is
...
walk, yet capable of directed connectivity~[9]. Since only phase around cycles is
...
quantum observable that no symmetric classical measure --- communicability $e^{A}$~[8],
diffusion $e^{-Lt}$, or the average mixing matrix~[1] --- can express on the same graph.
...
preserves only generic modes and destroys within-cluster residue resolution~[4]. We use $K = 32$--$64$ for analysis and
...
a quantum stochastic walk~[5] weights coherent and incoherent channels by a single
parameter $p \in [0,1]$:
\begin{equation}
\frac{d\rho}{dt}
 = -\,i(1-p)\,[H,\rho]
...
chance as $p \to 1$. Environment-assisted transport predicts an interior optimum~[7];
...
\midrule
KRAS G12C       & 4OBE & 6OIM & Cryptic Switch-II pocket (sotorasib)            & [15]\\
BCR-ABL1        & 1OPL & 5MO4 & Distal myristoyl pocket (asciminib)             & [16]\\
Cardiac myosin  & 5TBY & 6C1H & Site stabilising the super-relaxed state        & [17]\\
...
$e^{A}$~[8], diffusion $e^{-Lt}$, GNM centrality, bond-to-bond propensity~[3]
as the strongest published method here, PocketMiner ($0.87$)~[13] as the learned
...
\lead{Metrics of success.} Discrimination against the published $0.87$~[13];
