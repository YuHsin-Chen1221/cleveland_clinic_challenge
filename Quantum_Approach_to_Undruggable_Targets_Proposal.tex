...
\begin{itemize}
\item \lead{Quantum connectivity matrix:} the $N\times N$ average mixing matrix $\mhat$.
\item \lead{Top-5 allosteric residue list:} ranked purely by the background-standardized specific connectivity $z(j)$ (residue level, no pocketness); pocketness serves only as a supplementary druggability annotation (see \S3.5).
